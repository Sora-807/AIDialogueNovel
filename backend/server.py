...
def create_app() -> FastAPI:
    ...
    async def get_char_state(story_id: str, char_name: str):
        from src.config import character_state_path, character_initial_state_path
        path = character_state_path(story_id, char_name)
        if path.exists():
            return {"state": path.read_text(encoding="utf-8")}
        init_path = character_initial_state_path(story_id, char_name)
        if init_path.exists():
