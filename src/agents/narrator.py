...
- manage_stage(action, name, hint)：管理舞台上的角色。action: 'enter'（入场）/ 'exit'（退场）。
  name: 角色名。hint: 仅 enter 时需要——私密发给该角色。入场后必须 pick_speaker 让他说话。
...
2. 按小节顺序推进。切换小节时：先 speak 描述场景转换，再用 manage_stage 调整舞台
3. 如有需要，用 read_info 查阅世界观或角色状态
4. 确认理解后开始——写开场旁白或直接 pick 第一个发言人
...
Author 在角色安排中给每个角色标注了层级——你必须严格遵守：

**主线**：可以自由 pick 发言，正常分配对话轮次。

**过场**：只有少量戏份。Author 给了**台词方向**——在 pick_speaker 的导演提示中传递这个方向，角色用自己的风格表达。只分配一次 pick，之后用 manage_stage exit 让他退场。

**仅被提及**：不在角色安排中。不出场、不收到消息、不 pick。

## 舞台管理

你用 manage_stage 控制谁在场上。

入场完整流程：speak 描述入场 → manage_stage enter → **pick_speaker 把发言权交给刚入场的角色**
退场完整流程：speak 描述退场 → manage_stage exit（之后不再 pick 该角色）

manage_stage 只改变舞台，不等于角色发言。**入场后必须 pick_speaker 他才能出声**。切换立刻生效——之后的 speak/pick 只作用于舞台上的角色。"""
...
def manage_stage(action: str, name: str, hint: str = "") -> str:
    ...
    name: 角色名。hint: 入场提示（仅 enter 时有效）——私密发给该角色，告诉他现在看到了什么、感受到了什么。
    退场不需要 hint。切换立即生效——之后的 speak 只发给新舞台上的角色。"""
    return "OK"
...
class NarratorAgent(BaseAgent):
    ...
    def validate_tool(self, tool_name: str, args: dict) -> tuple[bool, str]:
        if tool_name == "speak":
            ...
            if action == "enter" and not args.get("hint", "").strip():
                return False, "enter 必须提供 hint——告诉角色入场时感知到的情况"
        return True, ""
